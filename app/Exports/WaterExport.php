...
class WaterExport implements FromArray, WithHeadings, WithStyles, WithTitle, ShouldAutoSize, WithCustomStartCell, WithEvents
{
    protected $month;
    protected $year;
    protected $locations;
    ...
    public function array(): array
    {
        ...
        for ($date = $startDate->copy(); $date <= $endDate; $date->addDay()) {
            ...
            if ($firstReading) {
                $previousValue = number_format($firstReading->previous_value, 2);
            }
            ...
            foreach ($this->locations as $location) {
                ...
                $row[] = $reading ? number_format($reading->current_value, 2) : '';
            }

            $row[] = $dailyUsage > 0 ? number_format($dailyUsage, 2) : '';
            $data[] = $row;
        }
    ...
    {
        $headings = ['No', 'Date (DD/MM/YY)', 'Previous Meter Reading (m3)'];

        foreach ($this->locations as $location) {
            $headings[] = $location->name;
        }

        $headings[] = 'Daily Water Use (m3)';
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('A1', 'Water Meter Reading Data');
    ...
    public function title(): string
    {
        ...
        return "Water {$monthName} {$this->year}";
    }
}